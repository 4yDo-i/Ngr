        ...
        if call.data == 'delete':
            ...
            for poll_item in range(0, len(poll_items)):
                for id_ in range(1, len(id_items)):
                    ...
                    bot.send_message(id_items[id_], f'{name} — {score}', parse_mode='html', reply_markup=markup)
                    #message_id_list.append(bot.send_message(id_items[id_], f'{name} - {score}', parse_mode='html',reply_markup=markup).message_id)
...
def callback(call):
    global poll_items
    if call.message:
        bot.send_message(call.message.id, f'Gau', parse_mode='html')
        for i in range(len(poll_items)):
            if call.data == f'0_{i}':
                pass
